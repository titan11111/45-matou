/* =========================
   ...
   addEventListener("keydown", (e)=>{ const k = KEYS[e.code]; if (k){ Input[k]=true; e.preventDefault(); }});
   ...
   class MapData {
     ...
     isBlocked(x,y){
       ...
       if (this.level===4 && !this.revealed && this.hiddenWallAt?.(x,y)) return true;
       return false;
     }
   ...
   class Scene { constructor(game){ this.game = game; } async start(){} update(dt){} draw(){} }
     ...
     draw(){
       ...
       for (let y=0; y<vh; y++){
         for (let x=0; x<vw; x++){
           ...
           // 5F 幻影：未可視なら壁を床に見せる
           if (this.map.level===4 && !this.map.revealed && this.map.hiddenWallAt(mx,my)){
